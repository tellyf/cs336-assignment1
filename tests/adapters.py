...
def run_embedding(
    ...
    """
    m = Embedding(num_embeddings=vocab_size, d_model=d_model)
    m.set_weights(weights)
...
def run_swiglu(
    ...
    f = FFN(d_model, d_ff)
    f.set_weights(w1_weight, w2_weight, w3_weight)
...
def run_multihead_self_attention(
    ...
    m.set_weights(q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)
    return m.forward(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    m = MultiHeadAttention(d_model, num_heads, theta, max_seq_len, token_positions)
    m.set_weights(q_proj_weight, k_proj_weight, v_proj_weight, o_proj_weight)    
    return m.forward(in_features)
...
def run_rope(
    ...
    """
    r = RoPE(theta=theta, d_k=d_k, max_seq_len=max_seq_len)
    return r.forward(in_query_or_key, token_positions=token_positions)
...
def run_transformer_block(
    ...
    """
    tbk = TransformerBlock(d_model, num_heads, d_ff, theta, max_seq_len)
    tbk.set_weights(weights)
    return tbk.forward(in_features)
...
def run_transformer_lm(
    ...
    for i in range(num_layers):
        ...
        lm.transformer_blocks[i].set_weights(tbw)
    lm.embedding.set_weights(weights['token_embeddings.weight'])
    lm.rms_norm.set_weights(weights['ln_final.weight'])
    lm.lm_head.set_weights(weights['lm_head.weight'])
