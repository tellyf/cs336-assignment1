...
def get_tokenizer_from_vocab_merges_path(
    ...
    mp = gpt2_bytes_to_unicode() #256 entry only [id to byte] # single byte unicode?
    gpt2_byte_decoder = {v: k for k, v in mp.items()} # byte to id
    with open(vocab_path) as vocab_f:
        # 50k (50257) entries. [string to index] different from gpt2 byte to unicode
...
def test_roundtrip_empty():
    """ Test that an empty string can be encoded and decoded without errors. """
